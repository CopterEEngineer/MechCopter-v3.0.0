...
public:
	...
	inline void output(ofstream &outfile) {
		for (int i = I0; i <= I1; ++i) { outfile << v_p[i] << endl; }
		outfile.close();
	}
	...
	inline void output(int i0, int i1, string filename) {
		...
		for (int i = i0; i <= i1; ++i) { OutFile << v_p[i - I0] << endl; }
		OutFile.close();
	}
...
inline Matrix1<float> atan2(const Matrix1<float> &A, const Matrix1<float> &B) {
	...
	for (int i = anv - 1; i >= 0; --i) {
		m = A.v_p[i];
		n = B.v_p[i];
		if (((m > 0) && (n > 0)) || ((m > 0) && (n < 0))) { temp.v_p[i] = atan(m / n); }
		else if ((m < 0) && (n > 0)) { temp.v_p[i] = atan(m / n) + PI; }
		else if ((m < 0) && (n < 0)) { temp.v_p[i] = atan(m / n) - PI; }
		else if ((m == 0) && (n > 0)) { temp.v_p[i] = 0.0; }
		else if ((m == 0) && (n < 0)) { temp.v_p[i] = PI; }
		else if ((m > 0) && (n == 0)) { temp.v_p[i] = PI/2; }
		else if ((m < 0) && (n == 0)) { temp.v_p[i] = -PI/2; }
		else { temp.v_p[i] = 0.0; }
#ifdef _DEBUG
